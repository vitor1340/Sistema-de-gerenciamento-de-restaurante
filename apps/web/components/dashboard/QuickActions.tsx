...
const ACTIONS_ESTATICAS: { icon: LucideIcon; title: string; subtitle: string }[] = [
...
export function QuickActions({ restauranteSlug }: { restauranteSlug: string }) {
  return (
    <div className="rounded-2xl border border-border bg-surface p-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <span>
            <span className="block text-xs font-semibold text-ink-primary">Visualizar loja</span>
            <span className="block text-[11px] text-ink-secondary">Como seus clientes veem</span>
          </span>
        </Link>

        {ACTIONS_ESTATICAS.map((action) => (
          <button
            key={action.title}
            className="flex items-start gap-2 rounded-xl border border-border p-3 text-left transition hover:border-brand/40 hover:bg-brand-soft"
          >
